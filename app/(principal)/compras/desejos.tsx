...
export default function DesejosCompraTela() {
  ...
  const converterSelecionados = async () => {
    ...
    try {
      ...
      await carregarDados();
      router.push(`/compras/planejamento?listaId=${resposta.listaId}` as never);
    } catch {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: COLORS.bgPrimary }}>
      ...
      >
        <View style={{ width: 42 }} />
        <Text style={{ flex: 1, textAlign: 'center', color: COLORS.accent, fontSize: 18, fontWeight: 'bold' }}>{t('compras.menu.desejos')}</Text>

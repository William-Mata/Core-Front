...
type ModuloDashboardId = 'financeiro' | 'compras';
...
function NumeroAnimado({
  ...
  deveAnimar,
  duracaoMs = 700,
}: NumeroAnimadoProps): ReactElement {
  const animacaoValor = useRef(new Animated.Value(0)).current;
  const [valorExibido, setValorExibido] = useState(() => Math.max(0, Math.round(valorFinal)));
  const animacaoJaExecutada = useRef(false);

  useEffect(() => {
    const valorNormalizado = Number.isFinite(valorFinal) ? Math.max(0, Math.round(valorFinal)) : 0;
    const ambienteTeste = process.env.NODE_ENV === 'test';

    if (ambienteTeste) {
      animacaoJaExecutada.current = true;
      setValorExibido(valorNormalizado);
      return;
    }
    ...
    const idListener = animacaoValor.addListener(({ value }) => {
      if (!componenteAtivo) return;
      setValorExibido(Math.round(value * valorNormalizado));
    });
  ...
  return (
    <Text style={{ color: cor, fontSize: tamanhoFonte, fontWeight: pesoFonte }}>
      {new Intl.NumberFormat(i18n.resolvedLanguage || i18n.language || 'pt-BR').format(valorExibido)}
    </Text>
  );
}
...
export default function Dashboard() {
  ...
  const [modulosExpandidos, setModulosExpandidos] = useState<Record<ModuloDashboardId, boolean>>(ESTADO_INICIAL_MODULOS_DASHBOARD);
  ...
  useEffect(() => {
    ...
    try {
      const valorSalvo = window.localStorage.getItem(CHAVE_PERSISTENCIA_MODULOS_DASHBOARD);
      if (!valorSalvo) return;
      const parseado = JSON.parse(valorSalvo) as Partial<Record<ModuloDashboardId, boolean>>;
      setModulosExpandidos({
        financeiro: typeof parseado.financeiro === 'boolean' ? parseado.financeiro : true,
        compras: typeof parseado.compras === 'boolean' ? parseado.compras : false,
      });
    } catch {
      // Ignorar erro de leitura de persistencia
    }
  }, []);
  ...
  useEffect(() => {
    if (Platform.OS !== 'web' || typeof window === 'undefined') return;
    try {
      window.localStorage.setItem(CHAVE_PERSISTENCIA_MODULOS_DASHBOARD, JSON.stringify(modulosExpandidos));
    } catch {
      // Ignorar erro de persistencia
    }
  }, [modulosExpandidos]);
  ...
  }, [idiomaAtual, versaoCargaCompras]);
  ...
  const renderWidget = (id: WidgetId) => {
    ...
    if (idsWidgetsCompras.has(id as IdWidgetCompras)) {
      return (
        <RenderizadorWidgetsCompras
          idWidget={id as IdWidgetCompras}
          larguraTela={width}
          carregandoCompras={carregandoCompras}
          erroCompras={erroCompras}
          aoTentarNovamente={() => setVersaoCargaCompras((valorAtual) => valorAtual + 1)}
          traduzir={t}
          resumoComprasKpi={resumoComprasKpi}
          evolucaoMensalCompras={evolucaoMensalCompras}
          tiposComprasAgregados={tiposComprasAgregados}
          produtosMaisComprados={produtosMaisComprados}
          ultimasCompras={ultimasCompras}
          ultimosDesejos={ultimosDesejos}
          variacoesPrecos={variacoesPrecos}
          produtosComMelhorEconomia={produtosComMelhorEconomia}
          economiaPotencialTotal={economiaPotencialTotal}
          renderizarValorMonetario={({ valor, cor, tamanhoFonte, pesoFonte, deveAnimar, margemTopo }) => (
            <ValorMonetarioAnimado
              ...
              deveAnimar={deveAnimar}
            />
          )}
          renderizarNumeroAnimado={({ valor, cor, tamanhoFonte, pesoFonte, deveAnimar }) => (
            <NumeroAnimado
              valorFinal={valor}
              cor={cor}
              tamanhoFonte={tamanhoFonte}
  ...
  const idsWidgetsCompras = new Set<WidgetId>([
    'comprasResumoRapido',
    'comprasEvolucaoMensal',
    'comprasTipos',
    'comprasProdutosTop',
  ...
  const idsWidgetsFinanceiro = new Set<WidgetId>([
    'resumo',
    'graficoReceitasAreaSubarea',
    'graficoDespesasAreaSubarea',
    'graficoAnual',
  ...
  const estaCarregandoWidget = (id: WidgetId): boolean => {
    if (idsWidgetsCompras.has(id)) return carregandoCompras;
    return carregando;
  };

  const alternarModuloDashboard = (moduloId: ModuloDashboardId) => {
  ...
  const widgetCardsPorModulo = useMemo<Record<ModuloDashboardId, WidgetId[]>>(
    () => ({
      financeiro: ordemWidgets.filter((id) => idsWidgetsFinanceiro.has(id)),
      compras: ordemWidgets.filter((id) => idsWidgetsCompras.has(id)),
    }),
    [ordemWidgets],
  );

  const configuracoesModulos = useMemo(
    () => [
      { id: 'financeiro' as const, icone: 'cash-multiple' as const },
      { id: 'compras' as const, icone: 'cart-outline' as const },
    ],
    [],
  );

  const obterPosicaoWidgetNoModulo = (id: WidgetId, moduloId: ModuloDashboardId) => {
    const widgetsModulo = widgetCardsPorModulo[moduloId];
    return widgetsModulo.indexOf(id);
  };

  const moverWidgetNoModulo = (id: WidgetId, moduloId: ModuloDashboardId, direcao: 'cima' | 'baixo') => {
    setOrdemWidgets((atual) => {
      const ehDoModulo = (widgetId: WidgetId): boolean => {
        if (moduloId === 'financeiro') return idsWidgetsFinanceiro.has(widgetId);
        return idsWidgetsCompras.has(widgetId);
      };

      const widgetsModuloAtual = atual.filter((widgetId) => ehDoModulo(widgetId));
      const posicaoAtual = widgetsModuloAtual.indexOf(id);
      if (posicaoAtual < 0) return atual;

      const alvoNoModulo = direcao === 'cima' ? posicaoAtual - 1 : posicaoAtual + 1;
      if (alvoNoModulo < 0 || alvoNoModulo >= widgetsModuloAtual.length) return atual;

      const widgetAlvo = widgetsModuloAtual[alvoNoModulo];
      const indiceOrigemGlobal = atual.indexOf(id);
      const indiceAlvoGlobal = atual.indexOf(widgetAlvo);
      if (indiceOrigemGlobal < 0 || indiceAlvoGlobal < 0) return atual;

      const nova = [...atual];
      [nova[indiceOrigemGlobal], nova[indiceAlvoGlobal]] = [nova[indiceAlvoGlobal], nova[indiceOrigemGlobal]];
      return nova;
    });
  };
  ...
  const renderizarCardWidget = (id: WidgetId, moduloId: ModuloDashboardId) => {
    ...
    return (
      <View key={id} {...webProps} testID={`dashboard-widget-${id}`} style={{ backgroundColor: COLORS.bgTertiary, borderWidth: 1, borderColor: widgetArrastando === id ? COLORS.borderAccent : COLORS.borderColor, borderRadius: 12, padding: 14, opacity: widgetArrastando === id ? 0.72 : 1 }}>
        ...
        {estaCarregandoWidget(id) ? renderWidgetCarregando() : renderWidget(id)}
      </View>
    );
  };
            ...
            return (
              <View key={modulo.id} style={{ backgroundColor: COLORS.bgTertiary, borderWidth: 1, borderColor: COLORS.borderColor, borderRadius: 12, overflow: 'hidden' }}>
                <TouchableOpacity
                  ...
                  accessibilityState={{ expanded: moduloExpandido }}
                  onPress={() => alternarModuloDashboard(modulo.id)}
                  style={{ padding: 14, backgroundColor: COLORS.bgSecondary, borderBottomWidth: moduloExpandido ? 1 : 0, borderBottomColor: COLORS.borderColor }}
                >
                  <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                    <View style={{ flex: 1 }}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                        <MaterialCommunityIcons name={modulo.icone} size={16} color={COLORS.textPrimary} />
                        <Text style={{ color: COLORS.textPrimary, fontSize: 14, fontWeight: '700' }}>
                          {t(`dashboard.modulos.${modulo.id}.titulo`)}
                        </Text>
                      </View>
                      <Text numberOfLines={2} style={{ color: COLORS.textSecondary, fontSize: 11, marginTop: 6 }}>
                        {resumoWidgets}
                      </Text>
                    </View>
                    <View style={{ width: 30, height: 30, borderRadius: 999, borderWidth: 1, borderColor: COLORS.borderColor, alignItems: 'center', justifyContent: 'center', backgroundColor: COLORS.bgTertiary }}>
                      <Text style={{ color: COLORS.textPrimary, fontSize: 12, fontWeight: '700' }}>{moduloExpandido ? '\u2212' : '+'}</Text>
                    </View>
                  </View>
                </TouchableOpacity>
                ...
                {moduloExpandido ? (
                  <View style={{ padding: 12, gap: 12 }}>
                    {widgetsModulo.length > 0 ? widgetsModulo.map((id) => renderizarCardWidget(id, modulo.id)) : (
                      <View style={{ backgroundColor: COLORS.bgSecondary, borderWidth: 1, borderColor: COLORS.borderColor, borderRadius: 10, padding: 12 }}>
                        <Text style={{ color: COLORS.textSecondary, fontSize: 12 }}>{t('dashboard.modulos.semWidgetsConfigurados')}</Text>
                      </View>
                    )}
                  </View>
                ) : null}

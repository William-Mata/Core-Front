...
export default function GerenciadorAvisos() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: COLORS.bgPrimary }}>
      <View style={{ backgroundColor: COLORS.bgTertiary, paddingHorizontal: 16, paddingTop: 16, paddingBottom: 16, borderBottomWidth: 1, borderBottomColor: COLORS.borderColor, flexDirection: 'row', alignItems: 'center' }}>
        <TouchableOpacity onPress={() => router.back()} style={{ width: 42, alignItems: 'flex-start' }}><Text style={{ color: COLORS.accent, fontSize: 24 }}>{'\u2190'}</Text></TouchableOpacity>
        <Text style={{ flex: 1, textAlign: 'center', color: 'white', fontSize: 18, fontWeight: '600' }}>{'\uD83D\uDCE3'} {t('admin.avisos.titulo')}</Text>
        <TouchableOpacity onPress={() => router.back()} style={{ width: 42, alignItems: 'flex-end' }}><Text style={{ color: COLORS.accent, fontSize: 20 }}>{'\u2715'}</Text></TouchableOpacity>
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity onPress={() => router.push(`/admin/aviso?id=${item.id}`)} style={{ backgroundColor: COLORS.bgTertiary, marginBottom: 12, paddingHorizontal: 14, paddingVertical: 12, borderRadius: 10, borderLeftWidth: 3, borderLeftColor: getTipoBadgeColor(item.tipo) }}>
      ...
      <TouchableOpacity onPress={() => router.push('/admin/aviso')} style={{ position: 'absolute', bottom: 24, right: 24, backgroundColor: COLORS.accent, width: 56, height: 56, borderRadius: 28, justifyContent: 'center', alignItems: 'center', shadowColor: COLORS.accent, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 8 }}>

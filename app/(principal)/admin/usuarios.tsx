...
export default function FormUsuario() {
  ...
  const abrirNovo = () => {
    resetarFormulario();
    router.push('/admin/usuarios?novo=1');
  };

  const abrirEdicao = (usuario: UsuarioRegistro) => {
    router.push(`/admin/usuarios?id=${usuario.id}`);
  };
  ...
  if (!modoFormulario) {
    return (
      <View style={{ flex: 1, backgroundColor: COLORS.bgPrimary }}>
        <View style={{ backgroundColor: COLORS.bgSecondary, borderBottomWidth: 1, borderBottomColor: COLORS.borderColor, paddingHorizontal: 16, paddingVertical: 12, flexDirection: 'row', alignItems: 'center' }}>
          ...
          <Text style={{ flex: 1, textAlign: 'center', color: COLORS.accent, fontSize: 18, fontWeight: 'bold' }}>{t('admin.usuarios.lista')}</Text>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: COLORS.bgPrimary }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.bgSecondary, borderBottomWidth: 1, borderBottomColor: COLORS.bgTertiary, paddingHorizontal: 16, paddingVertical: 12 }}>
        <View style={{ width: 42 }} />
        <Text style={{ flex: 1, textAlign: 'center', color: COLORS.accent, fontSize: 18, fontWeight: 'bold' }}>{emEdicao ? t('admin.usuario.editar') : t('admin.usuario.novo')}</Text>
        <TouchableOpacity onPress={voltarParaLista} style={{ width: 42, alignItems: 'flex-end' }}><Text style={{ color: COLORS.textSecondary, fontSize: 24 }}>{'\u2715'}</Text></TouchableOpacity>
      </View>
